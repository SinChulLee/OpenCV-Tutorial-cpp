...
int main(int argc, char *argv[])
{
    ...
    String window_name_of_video_dilated_with_5x5_kernel = "Video dilated with 5 x 5 kernel";

    // 위의 names를 가진 window 생성
    namedWindow(window_name_of_original_video, WINDOW_NORMAL);
    namedWindow(window_name_of_video_dilated_with_5x5_kernel, WINDOW_NORMAL);
    ...
    while (true)
    {
        ...
        // 5x5 kernel을 가진 프레임 침식
        Mat frame_dilated_with_5x5_kernel;
        dilate(frame, frame_dilated_with_5x5_kernel, getStructuringElement(MORPH_RECT, Size(5, 5)), Point(-1, -1), 2);

        // 생성된 windows에 프레임 표시
        imshow(window_name_of_original_video, frame);
        imshow(window_name_of_video_dilated_with_5x5_kernel, frame_dilated_with_5x5_kernel);
